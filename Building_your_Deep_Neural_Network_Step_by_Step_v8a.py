...
def L_model_forward(X, parameters):
    ...
    for l in range(1, L):
        A_prev = A 
        ### START CODE HERE ### (≈ 2 lines of code)
        A, caches = linear_activation_forward(A_prev, parameters["W" + str(l)], parameters["b" + str(l)], activation = "relu")
        ### END CODE HERE ###
...
def compute_cost(AL, Y):
    ...
    cost = -((np.sum(np.dot(Y, np.log(AL).T))/m )+ (np.sum(np.dot((1-Y), np.log(1-AL).T)))/m)
    ### END CODE HERE ###
